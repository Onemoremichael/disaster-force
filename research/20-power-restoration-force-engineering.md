...
That workbook is now complete at `R2+`. See [PRG-100 configuration model](21-power-configuration-model.md) and the editable [power-restoration engineering workbook](../models/power-restoration-engineering.xlsx). It expands the original sheet specification to 17 sheets so transformation, construction, fuel, thermal effects and evidence gates remain independently inspectable.
