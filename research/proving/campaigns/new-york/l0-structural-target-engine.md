...
The [moment-reconciliation successor](l0-moment-reconciliation-engine.md) proves that the published covariance blocks, marginal standard errors and exact additive identities can coexist, eliminating every interior spread failure. Its Gaussian states nevertheless violate nonnegativity or subset relations in roughly half of raw rows; one-sided repair shifts centers and tails. The [factorized atomic successor](l0-atomic-basis-registration.md) proves exact point-support feasibility but also extreme nonidentification. The next admissible pass is distributional calibration over that support, not donor fitting. Architecture access remains prohibited.
