...
The subsequent [target-engine ceiling audit](l0-target-engine-ceiling-audit.md) draws 36,000 candidate vectors and rejects all three ensemble cases at the registered ceiling. No continuous donor weights or integer household counts exist. The compiler has not been subjected to donor-fit, privacy or independent-reproduction gates. Public building-class capacity assignment remains unimplemented. Medical-power need, caregiver relation, exact household–building binding, floor, elevator and water-pump operability, indoor thermal state and live service status remain `UNKNOWN`. Borough remains an unapproved service-territory proxy, causal-feasibility rules remain open, and external salted holdout custody does not exist.

The subsequent [structural-coordinate engine](l0-structural-target-engine.md) draws coherent totals, complements and shares before projection and eliminates excessive repair, but it still fails tail stability and published-margin calibration. The [moment-reconciliation successor](l0-moment-reconciliation-engine.md) preserves official covariance and additive identities yet fails at nonlinear civil boundaries. The [factorized atomic basis](l0-atomic-basis-registration.md) then proves exact point-support feasibility while exposing 1,525 latent directions per catchment. The next admissible pass is a calibrated distribution over that support—not hierarchical donor fitting and not an architecture run. Every downstream output remains zero unless an ensemble is admitted. No `NCAC` evidence gate advances and no rendering is authorized.
