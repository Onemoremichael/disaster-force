...
The next admissible pass is a structural-coordinate target engine—not hierarchical donor fitting and not an architecture run. It must draw coherent totals, complements and shares before projection, repeat the registered nested convergence and repair-distance tests, and preserve every downstream output at zero unless an ensemble is admitted. No `NCAC` evidence gate advances and no rendering is authorized.
