...
Pass 52 [registers the factorized atomic support](l0-atomic-basis-registration.md) and proves exact point feasibility without supplying the required probability distribution. Pass 53 then [audits the atomic moment cone](l0-atomic-moment-cone-audit.md), adds authoritative under-five age detail and proves exact cloning of estimator covariance as state covariance impossible at seven nonnegative boundary margins. The next admissible method is a preregistered atomic measurement-error posterior `v3C0`. The Gaussian reconciler remains a useful linear benchmark and impossibility diagnostic, but it is rejected as target authority. Architecture access and renderings remain prohibited.
