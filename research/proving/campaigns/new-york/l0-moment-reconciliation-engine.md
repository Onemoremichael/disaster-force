...
Pass 52 [registers the factorized atomic support](l0-atomic-basis-registration.md) and proves exact point feasibility without supplying the required probability distribution. The next admissible pass is therefore `v3B1`: a stochastic calibration over feasible atomic states tested against the same ceiling. The Gaussian reconciler remains a useful linear benchmark and an impossibility diagnostic, but it is rejected as the target authority. Architecture access and renderings remain prohibited.
