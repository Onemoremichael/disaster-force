...
No generated state exists in this registration. The subsequent [PUMS donor custody and support audit](l0-pums-donor-support-audit.md) completes official source acquisition, exact verification, relational checks and the empirical local → adjacent → citywide support ladder without generating a target. The first [coherent target-engine implementation](l0-target-engine-ceiling-audit.md) then reaches its registered `K=1,000` ceiling without admitting any covariance case: all 36,000 candidates are feasible integers, but selected tails remain unstable and thirteen rows require excessive repair. The next gate is a new structural-coordinate target method, not donor fitting. Formal `L0` still requires an admitted target ensemble, tier-labeled household fit, external salted holdout custody, operational building and medical-power evidence or bounded unknown-state treatment approved for the test, service-territory authority, causal feasibility, alternative dependence cases and independent reconstruction of the earlier New York lineage.
