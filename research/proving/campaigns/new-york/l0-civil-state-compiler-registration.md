...
No generated state exists in this registration. The subsequent [PUMS donor custody and support audit](l0-pums-donor-support-audit.md) completes official source acquisition, exact verification, relational checks and the empirical local → adjacent → citywide support ladder without generating a target. The first [target-engine implementation](l0-target-engine-ceiling-audit.md) reaches its registered `K=1,000` ceiling without admitting a covariance case: all 36,000 candidates are feasible integers, but selected tails remain unstable and thirteen rows require excessive repair. The [structural successor](l0-structural-target-engine.md) removes that repair defect while failing tail stability and published-margin calibration. The [moment-reconciliation engine](l0-moment-reconciliation-engine.md) closes the linear covariance problem but shows that Gaussian states require distribution-changing repair at nonlinear civil boundaries. The [factorized atomic registration](l0-atomic-basis-registration.md) proves exact point feasibility but no stochastic authority. The next gate is calibration of a distribution over feasible atomic states, not donor fitting. Formal `L0` still requires an admitted target ensemble, tier-labeled household fit, external salted holdout custody, operational building and medical-power evidence or bounded unknown-state treatment approved for the test, service-territory authority, causal feasibility, alternative dependence cases and independent reconstruction of the earlier New York lineage.
