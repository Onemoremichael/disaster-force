...
Passes 221–230 triggered the second ten-pass checkpoint. [Adversarial Review HR-02](human-review-221-230.md) finds two valid completed five-pass branches, no invalid verdict and no branch that should be killed. It releases the interval, resets the consecutive no-`KILL` counter and registers `AVA` before Pass 231. Pass 231 closes the first `AVA` item; the counter is now one and no review blocker remains.
