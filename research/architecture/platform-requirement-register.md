...
Passes 27–30 add the first force-level configuration family. These entries govern what transport and utility platforms must carry; they are not vehicles themselves.
...
| NRL | National Receiving Lattice | Admitted requirement | Convert fixed sites, surviving buildings and local stocks into maintained, hazard-independent receiving capacity for mobile service formations | Pass 30 prepared-receiver case imports 20% of district occupied-area mass and screens the tenth pod below hour 72; national inventory, survivability and cost are open | site classes and catchments; parcel/building passports; hazard covariance; inspection cycles; utility interfaces; host agreements; workforce; inventories; exercises; alternative when receiver fails | R1 |
...
The immediate cross-cutting gate is now the mobile–fixed division of the `PCC-D1`. The complete transformational district remains 48.05 t above its air-compatible mass budget and needs 2,000 builders when all fifty districts are erected concurrently. The next force object is the National Receiving Lattice: a hazard-diverse, maintained network of sites, structures, interfaces, stocks, records and people that can accept a mobile pod without becoming an assumed local dependency. A parallel rival-technology program must reduce district mass, volume, labor and commissioning time. Vehicle geometry downstream of this division remains provisional.
