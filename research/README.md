...
> A **district** is the smallest repeatable occupied system. An **`NCAC`** is the smallest formation that owns the seam between safe local service and safe movement; `NCAC-35` denotes a 35,000-person demand class, not a jurisdiction. A **pod** is the smallest effect-complete deployable increment. A **campaign cell** is the smallest independently supported origin-to-effect path. A **force** is the set of certified cells, fixed capacity, commands, sustainment, and regeneration that can meet the concurrency standard.
