...
The [Harris County continuity and selective-mobility campaign](campaigns/harris-county-continuity-and-selective-mobility-campaign.md) then joins those layers in one metropolitan geography. Its corrective architecture wins all 300 paired broad worlds but clears the complete rule in only 55.3 percent. The result defines the `NCAC` as the next formation to configure: a protected district joined to building access, accessible pickup, local route assurance, transfer, rights, and return.
...
Five nested units now anchor the force:

> A **district** is the smallest repeatable occupied system. A **neighborhood continuity and access cell** is the smallest formation that owns the seam between safe local service and safe movement. A **pod** is the smallest effect-complete deployable increment. A **campaign cell** is the smallest independently supported origin-to-effect path. A **force** is the set of certified cells, fixed capacity, commands, sustainment, and regeneration that can meet the concurrency standard.
