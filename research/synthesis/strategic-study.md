...
The factorized atomic registration now supplies the missing semantic support. It defines 596 lawful person cells, 960 occupied household-unit cells and one vacant-unit cell per catchment, and finds exact integer witnesses for all 456 published margins. Yet the aggregation operator's rank 32 leaves 1,525 latent directions; none of nine tested operational joints is point-identified. Four impossible states also pass every inherited aggregate rule before the atomic support rejects them. The next engine must calibrate a distribution over these feasible states, not select one witness. Adding samples, choosing a favorable completion, sampling the polytope without disclosed regularization or relaxing the gate would not create evidence.

Formal freeze still refuses to close: no target ensemble is admitted; deterministic unsalted holdouts are inferable; medical-power need below county, exact household–building binding, occupancy and operational building state remain unmeasured; the registered donor compiler has not run or been independently reproduced; service territories and causal feasibility remain incomplete; and no independent team has reproduced the full operator lineage. No New York architecture outcome has been generated.
