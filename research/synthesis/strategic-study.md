...
- Harris County now joins building proxies, damaged-road restoration, queues, destination admission, and return at metropolitan scale, and `NCAC-35` converts one analytical catchment into a task, loss, workforce, equipment, utility, logistics, rights, and cost configuration. None of those layers has representative operational evidence; parcel/building detail and named destinations remain absent.
...
The immediate task is a preregistered second metropolitan campaign and a representative 168-hour `NCAC` experiment centered on disconnected household continuity and building access. It should replace the Harris-derived analytical catchment with named building and service-point alternatives, explicit occupations and qualifications, accessible vehicle cycles, route classes, electrical one-lines, water and residual balances, maintenance, degraded communications, rights cases, destination interfaces, and return. The full mission should remove one five-district cluster, one route group, one collection squadron, commercial communications, and selected utility components while requiring both effect and household-authority clocks to close.

In parallel, the cell should be split into alert, service-entry, expansion, sustainment, and relief echelons; transfer nodes into prepared, mobile, and expeditionary variants; and local continuity districts across heat, smoke, flood, grid-loss, and building-access states. The Harris workforce and lifecycle frame should be rebuilt from accepted task evidence before any national multiplication. Results should feed rival technology and complete-mission trials rather than prematurely stabilize a fleet plan.
