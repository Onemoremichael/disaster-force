...
The later [heat-blackout power and thermal service study](../../force/projection/heat-blackout-power-and-thermal-service-cell.md) advances the `P-5` mission boundary from a qualitative thermal-work concept to an `R2+` occupied-district competitor. It derives a 50,000-person service bundle, a twelve-cell `PTS-50` upper comparator, building-interface labor, six-hour protection trajectories, sixteen degraded worlds and a national program screen. It does not advance `P-5` or any `PTS-5` hardware to `R3`: the energy wet load, building physics, human rates and twenty-four representative gates remain open.

The [`PTS-5` lineage competition](../../force/projection/pts5-energy-and-heat-rejection-lineage-competition.md) then replaces the cell's generic source with six complete energy and rejection logics. It establishes a common service spine and four energy echelons, raises modeled receiver-side rejection to 2.96–5.69 MWth and demonstrates that storage, liquid, gas, local, prepared and strategic firm sources occupy different operating envelopes. The mixed upper comparator advances only to geometry competition; twenty-eight gates still deny `R3` and rendering.

The [`PTS-5` deployment-geometry competition](../../force/projection/pts5-deployment-geometry-competition.md) isolates that next boundary. It holds the rapid-bridge service payload constant while road, rail, shallow-water, prepared-anchor and tri-modal hosts compete across twenty-three access, site, thermal, labor and denial worlds. The hosts pass seventeen, fourteen, eighteen, twenty and seventeen worlds, but none reaches `R3`: thirty-four gates still deny lineage–geometry coupling, complete-group closure and rendering.
