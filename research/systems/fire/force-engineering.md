...
See [Fire response force configuration model](configuration-model.md) and the editable [fire response force engineering workbook](../../../models/systems/fire/fire-response-force-engineering.xlsx). `IIN` and `IFCG` advance to `R2+`; all 18 evidence gates remain open and no rendering is authorized.
