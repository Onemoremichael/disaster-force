...
| Compound assurance | `DCP-2R` | an adaptive physical payload grammar and spare carriers preserve a complete service vector after a carrier loss and one further tile loss |
...
The new [`DCP-2R` common carrier and mission compiler](research/force/projection/dcp2r-common-carrier-and-mission-compiler.md) then overturns a second intuition: more vehicles do not create functional reserve when every payload remains exact and specialized. The selected behavior reference physically codes the civil effect across forty 15.875-tonne multifunction tiles in five independently implemented service families. Three atmospheric carriers are loaded so any two retain all forty tiles; five terrain carriers are loaded so any four do the same. After that carrier loss, one further tile may fail while every family still supplies its hundred-unit floor. This is a $4.941T, 439-article, 906-deployed-person hypothesis—not an admitted design—and all 160 evidence gates remain open.
...
The common mission compiler now shows that the original `DCP-2` is a valid single-cut lower bound but not a compound-assurance formation. Adding carriers without changing its exact specialized blocks still fails. The provisional `DCP-2R` reference therefore changes both payload and fleet: an `ESK-40` adaptive service kernel distributes forty multifunction tiles across three atmospheric or five terrain carriers, and any surviving air pair or terrain quartet retains the complete service vector after one additional tile loss. Twenty-eight declared inside-guarantee worlds pass; four harder cases are labeled boundary probes rather than claimed performance.

The next force move is physical, not illustrative: close dry mass, wet mass, connectors, containment, service interfaces, control, custody and failure behavior for each of the five tile families in both independent mode lineages; then assemble representative ten-tile and twenty-tile loads and destroy the formation through carrier and tile casualties at a complete receiver. Exterior concept art remains blocked until those articles give the carrier–payload interfaces representative geometry.
