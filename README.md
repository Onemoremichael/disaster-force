...
The campaign has deliberately produced no architecture result. Official geography, a 12,000-world experimental surface, demographic and building inputs, a household-preserving compiler specification and a PUMS donor-support audit are registered. The first target engine failed because it asked integer projection to repair independently drawn civil totals. A structural-coordinate successor now generates all 36,000 candidates coherently before projection: every row is a valid integer and repair is negligible. It still admits no ensemble. All three uncertainty structures miss selected-tail stability, and each fails published-margin calibration because a one-direction hierarchy can accumulate cross-table uncertainty; housing-unit spread reaches about `1.8×` its official standard error.

That second negative result is sharper. Row coherence, numerical repair, sample stability and marginal calibration are different claims. Passing the first two cannot compensate for failing the latter two. The next engine must reconcile published moments across the ensemble or construct a defensible latent atomic-cell model; reversing which table sits upstream would only relocate the distortion. Donor fitting and every downstream outcome remain sealed. The [active frontier](research/frontier.md) records this decision and the next admissible work. The [New York campaign dossier](research/proving/campaigns/new-york/README.md) holds the full ordered evidence chain.
