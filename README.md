...
The new audit also overturns the intended stochastic objective. Seven official margins have point estimate zero and positive sampling error. No distribution over nonnegative civil states can have both mean zero and positive variance, so exact cloning of published points and estimator covariance as state moments is mathematically impossible. The covariance remains authoritative in a measurement likelihood, not as variation among true populations. The next engine must preregister `p(x | ŷ) ∝ p(ŷ | A x,V)p₀(x)` on the atomic support, with frozen prior-sensitivity and posterior-predictive gates. Donor fitting and every downstream outcome remain sealed. The [active frontier](research/frontier.md) records this decision and the next admissible work. The [New York campaign dossier](research/proving/campaigns/new-york/README.md) holds the ordered evidence chain.
